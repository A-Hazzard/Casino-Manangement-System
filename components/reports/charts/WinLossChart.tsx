        ...
        <CardContent>
          <div className="h-64 flex items-center justify-center text-gray-500">
            <div className="text-center">
              ...
              <div className="text-xs">No win/loss data for the selected time period</div>
            </div>
          </div>
        </CardContent>
      </Card>
    ...
    <Card className={className}>
      <CardHeader>
        ...
        <div className="text-sm text-gray-600">
          Total: {formatCurrency(totalWinLoss)} | Peak: {formatCurrency(maxWinLoss)}
        </div>
      </CardHeader>
      <CardContent>
        <div className="h-64">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={data}>
              ...
              />
              <Tooltip
                formatter={(value: number) => [formatCurrency(value), 'Win/Loss']}
              ...
              >
                {data.map((entry, index) => (
                  <Cell key={`cell-${index}`} fill={getBarColor(entry.winLoss)} />
                ))}
              </Bar>
            </BarChart>
          </ResponsiveContainer>
