  ...
  const locationSuggestions = useMemo(() => {
    ...
    return locations
      .map((locationId, index) => ({
        locationId,
        locationName: locationNames?.[locationId] || locationId,
        index,
      }))
      .filter(item => item.locationName.toLowerCase().includes(searchLower))
      .slice(0, 10);
  }, [locations, locationNames, searchTerm]);

  // Handle focus - find location and highlight it
  const handleFocus = () => {
    if (!searchTerm || !scrollRef.current) return;

    const matchingLocation = locationSuggestions[0];
    if (matchingLocation) {
      // Find the index of the first data point for this location
      const index = filteredData.findIndex(item => {
        const displayName =
          locationNames?.[matchingLocation.locationId] ||
          matchingLocation.locationId;
      ...
      if (index !== -1) {
        ...
        const slotWidth = minWidth / filteredData.length;
                  ...
                  <div className="absolute z-50 mt-1 max-h-60 w-full overflow-auto rounded-md border bg-white shadow-lg">
                    {locationSuggestions.map(item => (
                      <button
                        ...
                        onClick={() => {
                          setSearchTerm(item.locationName);
                          setShowSuggestions(false);
                  ...
                  {focusedIndex !== null &&
                    ...
                    filteredData[focusedIndex] && (
                      <ReferenceArea
                        x1={
                          shouldShowTimes
                            ? (filteredData[focusedIndex].time as string) || ''
                            : (filteredData[focusedIndex].day as string)
                        }
                        x2={
                          shouldShowTimes
                            ? (filteredData[focusedIndex].time as string) || ''
