...
/**
 * Custom hook for Games Revenue data processing
 */
export function useGamesRevenueData({
  initialData,
  allMachines,
  selectedFilters,
  selectedGames,
}: UseGamesRevenueDataProps) {
  ...
  const aggregatedData = useMemo(() => {
    ...
    // Apply Top 5 / Bottom 5 / Highest Percent filters
    if (!selectedFilters.includes('all-games')) {
      if (
        ...
        gameData.sort((a, b) => b.totalGross - a.totalGross);
