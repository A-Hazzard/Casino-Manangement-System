...
export default function DashboardPageContent() {
  ...
  const prevGranularityRef = useRef<ChartGranularity | undefined>(undefined);

  /**
   * Fetches metrics data (totals, chart) and gaming locations.
   * It now distinguishes between a general data fetch and a granularity-only change
   * to avoid showing a skeleton on the financial cards unnecessarily.
   */
  useEffect(() => {
    ...
    const fetchMetrics = async () => {
      const isGranularityChangeOnly =
        prevGranularityRef.current !== chartGranularity &&
      ...
      try {
        // Only fetch locations if it's not a granularity-only change
        if (!isGranularityChangeOnly) {
