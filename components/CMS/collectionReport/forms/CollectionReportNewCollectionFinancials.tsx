      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
        ...
        <div>
          <label className="mb-2 flex items-center text-sm font-bold text-gray-700">
            Amount To Collect: <span className="ml-1 text-red-500">*</span>
            <CalculationHelp
          ...
          <p className="mt-1.5 text-xs font-medium leading-tight">
            ...
            financials.balanceCorrection.trim() === '' ? (
              <span className="flex items-center gap-1.5 rounded border border-amber-200 bg-amber-50 p-2 text-amber-600">
                ...
                Locked: Enter a Balance Correction first (even if 0) to unlock
                this field.
              </span>
            ) : (
              <span className="block rounded border border-blue-200 bg-blue-50 p-2 text-blue-700">
                Action: Count all physical cash collected from machines and
                enter the exact total here.
              </span>
            )}
          </p>
        </div>
        <div>
          <label className="mb-2 flex items-center text-sm font-bold text-gray-700">
            Balance Correction: <span className="ml-1 text-red-500">*</span>
            <CalculationHelp
          ...
          <p className="mt-1.5 text-xs font-medium leading-tight">
            {financials.collectedAmount.trim() !== '' ? (
              <span className="flex items-center gap-1.5 rounded border border-amber-200 bg-amber-50 p-2 text-amber-600">
                <Info className="h-3.5 w-3.5 shrink-0" />
                Note: Clear the 'Collected Amount' above if you need to
                re-adjust this field.
              </span>
            ) : (
              <span className="block rounded border border-gray-200 bg-gray-50 p-2 text-gray-600">
                Required: Set the opening balance or adjustments for this
                collection batch.
              </span>
            )}
          </p>
      ...
      <div className="mt-6 rounded-xl border-2 border-dashed border-blue-200 bg-blue-50/40 p-5 shadow-sm">
        ...
        <div className="grid grid-cols-1 gap-6 md:grid-cols-3">
          <div className="space-y-1">
            ...
            <p className="text-[10px] italic leading-tight text-blue-700/70">
          ...
          <div className="space-y-1 border-l border-blue-100 pl-4 md:border-l md:pl-6">
            ...
            <p
              className={`text-lg font-black ${Number(financials.previousBalance) < 0 ? 'text-red-600' : 'text-green-600'}`}
            >
        ...
        <div className="mt-5 border-t border-blue-100 pt-4">
          <div className="flex flex-col gap-2 rounded-lg border border-blue-50 bg-white/50 p-3">
            <p className="flex items-center gap-1.5 text-[11px] font-bold text-blue-900">
              <span className="inline-block h-1.5 w-1.5 rounded-full bg-blue-500"></span>
              The Reconciliation Breakdown:
            </p>
            <p className="rounded bg-blue-50/50 p-2 font-mono text-xs leading-relaxed text-blue-800">
              ...
              (Target)
              <span className="mx-2 font-bold text-blue-400">=</span>
              <span
            ...
            <p className="mt-1 text-[10px] italic text-gray-500">
              Note: The 'Balance Correction' set earlier acts as the opening
              anchor to reconcile this batch.
            </p>
          </div>
        </div>
      </div>
