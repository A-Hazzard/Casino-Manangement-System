...
type MobileCollectedListPanelProps = {
  ...
  hasChanges?: boolean; // For edit mode - true if new machines added or existing machines edited
  newMachineIds?: string[]; // IDs of newly added machines to show "NEW" badge
                        ...
                        <div className="flex flex-col gap-2">
                          <div className="min-w-0 flex-1">
                            ...
                            <ModernCalendar
                              ...
                              mode="single"
                              className="w-full"
                              maxDate={updateAllSasEndDate || new Date()}
                            />
                          </div>
                          <div className="min-w-0 flex-1">
                            <label className="mb-0.5 block text-[10px] font-medium text-gray-500">
                    ...
                    {sortedMachines.length === 0 && searchTerm ? (
                      ...
                      sortedMachines.map(machine => {
                        const hasVariation = variationMachineIds.some(
                          vid => String(vid) === String(machine.machineId)
                        );
                        const isNewMachine = newMachineIdsSet.has(
                          String(machine._id)
                        );
                        ...
                        const machineKey =
                          machine._id ||
                          machine.machineId ||
                          `idx-${sortedMachines.indexOf(machine)}`;
                        return (
                          <div
                            key={machineKey}
                            ...
                            <div className="flex items-start justify-between gap-4">
                              <div className="flex-1 space-y-2">
                                <div className="flex items-start justify-between">
                                  ...
                                  {isNewMachine && !hasVariation && (
                                    <div className="flex items-center gap-1 rounded-full bg-green-500 px-2 py-0.5 text-[8px] font-black uppercase text-white shadow-sm">
                                      NEW
                                    </div>
                                  )}
                                </div>
