...
> = ({ metric, useNetGross }) => {
  ...
  if (metric.sasGross === 'No SAS Data') {
    return (
      <span className="font-medium italic text-gray-500">No SAS Data</span>
    );
  }
  ...
  const adjustedSasGross = hasJackpotDeduction
    ? rawSasGross - jackpot
    : rawSasGross;
  ...
  const displayValue = fmt(
    hasJackpotDeduction ? adjustedSasGross : rawSasGross
  );

  if (!hasJackpotDeduction) {
    return (
      <span
        className={`font-medium ${rawSasGross < 0 ? 'text-red-600' : 'text-green-600'}`}
      >
        {displayValue}
      </span>
    );
      ...
      <Tooltip>
        <TooltipTrigger asChild>
          <div className="inline-flex cursor-help border-b border-dotted border-gray-400">
            <span
              className={`font-medium ${adjustedSasGross < 0 ? 'text-red-600' : 'text-green-600'}`}
            >
              {displayValue}
            </span>
          </div>
        ...
        <TooltipContent className="max-w-xs">
          <div className="space-y-1 text-xs">
            ...
            <p>
              SAS Gross (original):
              <span className="ml-1 font-bold">{fmt(rawSasGross)}</span>
            </p>
            <p>
              Jackpot:
            ...
            <p className="border-t border-gray-200 pt-1">
              Net SAS Gross:{' '}
              <span className="ml-1 font-bold">{displayValue}</span>
            </p>
          </div>
        </TooltipContent>
      </Tooltip>
