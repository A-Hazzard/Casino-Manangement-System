/**
 * Edit Licencee Modal Component
 * Modal for editing existing licencees with country and date information.
 *
 * Features:
 * - Licencee name and description editing
 * - Country selection
...
import { formatLicenceeDate } from '@/lib/utils/licencee';
import type { Country } from '@/lib/types/common';

type AdministrationEditLicenceeModalProps = {
  open: boolean;
  onClose: () => void;
  onSave: () => void;
  formState: {
...
function AdministrationEditLicenceeModal({
  open,
  onClose,
  onSave,
  formState,
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40">
      ...
      >
        ...
        <h2 className="mb-4 text-2xl font-bold">Edit Licencee</h2>
            ...
            <label className="mb-1 flex items-center gap-2 text-sm font-semibold">
              ...
              <div className="group relative">
                ...
                <div className="absolute bottom-full left-1/2 z-10 mb-2 -translate-x-1/2 transform whitespace-nowrap rounded bg-gray-800 px-2 py-1 text-xs text-white opacity-0 transition-opacity duration-200 group-hover:opacity-100">
                  Select the start date for this licencee
                </div>
              </div>
            </label>
            {formState.prevStartDate && (
              <div className="mb-1 text-xs text-gray-500">
                Previous: {formatLicenceeDate(formState.prevStartDate)}
              </div>
            )}
            <DateTimePicker
              date={
          ...
          <div>
            <label className="mb-1 flex items-center gap-2 text-sm font-semibold">
              ...
              <div className="group relative">
                <Info className="h-4 w-4 cursor-help text-gray-400" />
                <div className="absolute bottom-full left-1/2 z-10 mb-2 -translate-x-1/2 transform whitespace-nowrap rounded bg-gray-800 px-2 py-1 text-xs text-white opacity-0 transition-opacity duration-200 group-hover:opacity-100">
                  Select the expiry date for this licencee
                </div>
              </div>
            </label>
            {formState.prevExpiryDate && (
              <div className="mb-1 text-xs text-gray-500">
                Previous: {formatLicenceeDate(formState.prevExpiryDate)}
              </div>
            )}
            <DateTimePicker
              date={
