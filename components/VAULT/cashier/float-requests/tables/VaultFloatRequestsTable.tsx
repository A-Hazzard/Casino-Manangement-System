                ...
                return (
                  ...
                  >
                    <TableCell isFirstColumn className="text-center">
                      <Badge className={cn('px-2 py-0.5 text-[10px] border-none', isIncrease ? 'bg-green-600 text-white' : 'bg-red-600 text-white')}>
                        {isIncrease ? 'Increase' : 'Decrease'}
                      </Badge>
                    </TableCell>
                    <TableCell className={cn('text-center font-bold', isIncrease ? 'text-green-600' : 'text-red-600')}>
                      {isIncrease ? formatAmount(request.amount) : `-${formatAmount(Math.abs(request.amount))}`}
                    </TableCell>
                    {!showHistory && (
                      <>
          ...
          {requests.map(request => {
            ...
            return (
              <motion.div 
                ...
                className="h-full"
              >
                <Card className={cn("overflow-hidden border-l-4 shadow-sm h-full", isIncrease ? "border-l-green-600" : "border-l-red-600")}>
                  <CardContent className="p-4 space-y-3">
                    <div className="flex items-center justify-between">
                      <Badge className={cn('px-2 py-1 border-none', isIncrease ? 'bg-green-600 text-white' : 'bg-red-600 text-white')}>
                        {isIncrease ? 'Increase' : 'Decrease'}
                      </Badge>
                      <span className={cn("text-lg font-black", isIncrease ? "text-green-600" : "text-red-600")}>
                        {isIncrease ? formatAmount(request.amount) : `-${formatAmount(Math.abs(request.amount))}`}
                      </span>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-x-4 gap-y-2 text-xs">
                      ...
                      <div className="flex flex-col items-end">
                        ...
                        <span className="font-bold">{formatAmount(request.newFloat || 0)}</span>
                      </div>
