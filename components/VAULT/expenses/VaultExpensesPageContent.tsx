...
export default function VaultExpensesPageContent() {
  ...
  const [selectedExpense, setSelectedExpense] =
    useState<VaultTransaction | null>(null);
  ...
  const fetchExpenses = useCallback(async () => {
    try {
      ...
      const response = await axios.get(
        `/api/vault/expense?${params.toString()}`
      );
  ...
  const handleRecordExpense = async (data: {
    ...
    try {
      ...
      if (data.expenseDetails)
        formData.append('expenseDetails', JSON.stringify(data.expenseDetails));
      ...
      <div className="flex flex-col gap-6">
        ...
        >
          <div className="flex flex-col gap-6">
            <VaultManagerHeader
              title="Expenses"
              description="Manage vault expense records"
            >
                  ...
                  onClick={() => {
                    if (!hasActiveVaultShift) {
                      toast.error('Operation Blocked', {
                        description:
                          'You must start a vault shift before recording expenses.',
                      });
                      return;
                    }
                    if (isStaleShift) {
                    ...
                    if (!isVaultReconciled) {
                      toast.error('Reconciliation Required', {
                        description:
                          'Please perform the mandatory opening reconciliation before recording expenses.',
                      });
                      return;
                    }
                    setShowModal(true);
                            ...
                            <TableRow key={expense._id}>
                              ...
                              <TableCell className="text-right">
                                <Button
                                  variant="ghost"
                                  size="sm"
                                  className="h-8 px-3 font-bold text-violet-600"
                          ...
                          <div className="flex items-start justify-between">
                            ...
                            <div className="flex flex-col items-end gap-2">
                              ...
                              <Button
                                variant="ghost"
                                size="sm"
                                className="mt-auto h-7 px-2 text-[10px] font-bold text-violet-600"
                                onClick={() => setSelectedExpense(expense)}
                              >
