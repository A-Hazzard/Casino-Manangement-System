...
export default function UserModal({
  ...
  useEffect(() => {
    if (user) {
      ...
      setRoles(user.roles || []);
      setSelectedLocationIds(user.resourcePermissions?.["gaming-locations"]?.resources || []);
    } else if (open) {
      setIsLoading(true);
    }
  }, [user, open]);
  ...
  useEffect(() => {
    fetchAllGamingLocations().then((locs) => {
      ...
      const userLocationIds = user?.resourcePermissions?.["gaming-locations"]?.resources || [];
      if (userLocationIds.length > 0 && formattedLocs.length > 0) {
        setAllLocationsSelected(userLocationIds.length === formattedLocs.length);
      }
    });
  }, [user]);
                      ...
                      {isEditMode ? (
                        ...
                        <div className="text-center">
                          <div className="text-gray-700">
                            {roles && roles.length > 0 ? roles.map(role => ROLE_OPTIONS.find(r => r.value === role)?.label).join(", ") : "No roles assigned"}
                          </div>
                        </div>
                      )}
                    </div>
                          ...
                          <div className="text-gray-700">
                            {allLocationsSelected 
                              ...
                              : selectedLocationIds.length > 0 
                                ? selectedLocationIds.map(id => locations.find(l => l._id === id)?.name).filter(Boolean).join(", ")
