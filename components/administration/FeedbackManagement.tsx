...
import { Eye, MessageSquare, RefreshCw, Search, Save, Edit2, Trash2, RotateCcw } from 'lucide-react';
...
export default function FeedbackManagement() {
  ...
  const handleUpdateFeedback = async () => {
    ...
    try {
      ...
      console.log('🔍 Checking archived change:', { 
        currentArchived, 
        editArchived, 
        selectedFeedbackArchived: selectedFeedback.archived,
        areEqual: editArchived === currentArchived
      });
      
      if (editArchived !== currentArchived) {
        updateData.archived = editArchived;
        console.log('📝 Archived change detected - will update:', { 
          from: currentArchived, 
          to: editArchived 
        });
      } else {
        console.log('ℹ️ Archived unchanged, skipping update');
      }
      ...
      // Backend will automatically set reviewedBy and reviewedAt when status changes to reviewed or resolved

      console.log('📤 Sending update request:', updateData);

      // Use PATCH for partial updates to bypass potential full-document validation issues
      ...
      // Update local state - ensure archived is included
      const updatedFeedback = { 
        ...selectedFeedback, 
        ...data.feedback,
        archived: data.feedback.archived ?? false, // Explicitly ensure archived is set
      };
      console.log('✅ Updated feedback:', updatedFeedback);
      console.log('✅ Updated feedback archived:', updatedFeedback.archived);
      ...
      fetchInitialBatch(); // Refresh the list
    } catch (error) {
  ...
  const handleDeleteConfirm = async () => {
    ...
    setIsDeleting(true);
    try {
      // Use explicit method for delete
      const response = await fetch('/api/feedback', {
        method: 'DELETE',
        headers: {
          'Content-Type': 'application/json',
    ...
    try {
      ...
      const response = await fetch('/api/feedback', {
        ...
        body: JSON.stringify({
          ...
          archived: false, // Unarchive the feedback
        }),
      });
      ...
      // Update selected feedback if it's the one being restored
