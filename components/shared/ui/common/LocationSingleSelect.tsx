...
export default function LocationSingleSelect({
  ...
  // Always calculate dropdown width based on longest text to prevent truncation
  const [dropdownWidth, setDropdownWidth] = useState<number | undefined>(undefined);
  
  // Measure actual content width after dropdown opens using a hidden measurement element
  useEffect(() => {
    if (isOpen && typeof window !== 'undefined') {
      // Use requestAnimationFrame to ensure DOM has rendered
      requestAnimationFrame(() => {
        if (dropdownContentRef.current && buttonRef.current) {
          // Create a temporary element to measure the longest text
          const measureEl = document.createElement('div');
          measureEl.style.position = 'absolute';
          measureEl.style.visibility = 'hidden';
          measureEl.style.whiteSpace = 'nowrap';
          measureEl.style.font = window.getComputedStyle(buttonRef.current).font;
          ...
          // Add padding: checkbox (24px) + left padding (12px) + right padding (12px) + gaps (16px) + border (2px) + extra buffer (40px)
          const buttonWidth = buttonRef.current.offsetWidth;
          const calculatedWidth = Math.max(textWidth + 106, buttonWidth);
    ...
    <div className={`relative ${className}`} ref={dropdownRef}>
      ...
      {isOpen && (
        <div 
          ...
          className="absolute z-[70] mt-1 max-h-60 overflow-y-auto overflow-x-hidden rounded-md border border-gray-200 bg-white shadow-lg min-w-full"
          style={dropdownWidth ? { width: `${dropdownWidth}px`, minWidth: '100%' } : { minWidth: '100%', width: 'auto' }}
        >
          {/* Header with search */}
