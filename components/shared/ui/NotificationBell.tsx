...
export default function NotificationBell({
  ...
  const unreadCount = propUnreadCount ?? notifications.filter(n => n.status === 'unread').length;
  ...
  const NotificationListContent = (
    <>
      <div className="flex items-center justify-between px-4 py-3 border-b bg-white sticky top-0 z-10 rounded-t-xl">
        <div className="flex flex-col">
          <DropdownMenuLabel className="p-0 font-bold text-lg text-gray-900">
        ...
        <div className="flex gap-2 items-center">
          {!isSelecting && notifications.length > 0 && (
            <Button
              ...
              className="h-auto p-1 text-[11px] font-bold text-red-500 hover:text-red-600 hover:bg-red-50 uppercase"
            >
              Clear All
            </Button>
          )}
          <Button
            ...
            className={`h-auto p-1 text-[11px] font-bold uppercase flex items-center gap-1.5 rounded-md transition-colors ${
              isSelecting 
      ...
      <div className="px-4 py-2 flex justify-end bg-gray-50/50 border-b">
      ...
      {isSelecting && filteredNotifications.length > 0 && (
        <div className="bg-blue-50/80 px-4 py-2 border-b flex items-center justify-between">
          <div className="flex items-center gap-3">
            <Checkbox 
              ...
              id="select-all"
            />
            <label htmlFor="select-all" className="text-[11px] font-bold text-blue-700 cursor-pointer uppercase">
      ...
      {filteredNotifications.length === 0 ? (
        ...
        <div className="flex-1 overflow-y-auto divide-y bg-white min-h-0">
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2 mb-0.5">
                    ...
                    {notification.status === 'cancelled' && (
                      <Badge variant="outline" className="text-[9px] h-3 px-1 border-red-200 text-red-600 bg-red-50 font-black">
                        CANCELLED
                      </Badge>
                    )}
                  </div>
                  ...
                  <p className="mt-1.5 text-[9px] text-gray-400 font-black uppercase tracking-widest">
  ...
  const BellTrigger = (
    <Button variant="ghost" size="sm" className="relative group p-2 hover:bg-gray-100 rounded-full transition-colors">
      <Bell className="h-5 w-5 text-gray-700 group-hover:text-blue-600 transition-colors" />
      {unreadCount > 0 && (
        <Badge
          variant="destructive"
          className="absolute -right-0.5 -top-0.5 flex h-5 w-5 items-center justify-center p-0 text-[10px] font-black border-2 border-white shadow-sm"
        >
    ...
    <>
      {isMobile ? (
        <Dialog open={isOpen} onOpenChange={setIsOpen}>
          ...
          <DialogContent 
             className="max-h-[85vh] overflow-hidden p-0 flex flex-col"
        ...
        <DropdownMenu open={isOpen} onOpenChange={setIsOpen}>
          ...
          <DropdownMenuContent align="end" className="w-[400px] p-0 overflow-hidden flex flex-col max-h-[600px] shadow-2xl border-gray-200 rounded-xl">
            {NotificationListContent}
          </DropdownMenuContent>
        </DropdownMenu>
      )}
      ...
      <Dialog open={!!viewDetails} onOpenChange={(open) => {
        ...
        <DialogContent className="md:max-w-2xl p-0 overflow-hidden flex flex-col">
          ...
          {viewDetails ? (() => {
            ...
            return (
              <div className="flex-1 overflow-y-auto p-6 md:p-8 space-y-6 custom-scrollbar md:max-h-[85vh]">
                       ...
                       {!hasShortage && (
                         <div className="sm:hidden -mb-2">
                           <Button 
                              ...
                              className="w-full text-[11px] font-bold uppercase tracking-wider text-blue-600 border-blue-100 bg-blue-50/50"
                           >
                             {showVaultOnMobile ? 'Hide Vault Comparison' : 'Show Vault Comparison'}
                           </Button>
                         </div>
                       )}

                       {/* Left Column: Vault Stock */}
                        <div className={`${(!showVaultOnMobile && !hasShortage) ? 'hidden sm:block' : 'block'} space-y-3`}>
                          ...
                          <div className="grid grid-cols-1 gap-2">
                            {isEditing ? (
                                ...
                                editedDenominations.map((denom, index) => {
                                    ...
                                    const isOver = viewDetails.metadata?.requestType !== 'decrease' && requested > available;
