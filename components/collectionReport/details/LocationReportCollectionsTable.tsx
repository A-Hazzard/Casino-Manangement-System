...
type LocationReportCollectionsTableProps = {
...
export default function LocationReportCollectionsTable({
  metrics,
  paginatedMetrics,
  searchTerm,
  onSearchChange,
  ...
  totalPages,
  onPageChange,
}: LocationReportCollectionsTableProps) {
          ...
          <TableBody>
            {paginatedMetrics.map((metric) => (
              ...
              >
                ...
                <TableCell className="px-6 py-4 text-gray-600">{metric.dropCancelled || '0 / 0'}</TableCell>
        ...
        {paginatedMetrics.map((metric) => (
          ...
          >
            ...
            <div className="grid grid-cols-2 gap-y-3 text-sm">
              ...
              <MobileField label="Variation" value={metric.variation ?? 0} isCurrency isBold />
            </div>
          </div>
        ))}
      </div>
...
function SortableHeader({ label, field, currentField, direction, onClick }: {
  ...
  return (
    <TableHead
      ...
      className="cursor-pointer px-6 py-4 font-semibold text-gray-600 transition-colors hover:text-blue-600"
    >
...
function MobileField({ label, value, isCurrency = false, isBold = false }: {
  label: string;
  value: string | number;
  isCurrency?: boolean;
  isBold?: boolean;
  ...
  return (
    <div>
      ...
      <p className={`mt-0.5 font-medium ${isBold ? 'font-bold' : ''}`}>
