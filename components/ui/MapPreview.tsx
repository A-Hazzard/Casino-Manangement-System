...
const getValidLongitude = (geo: { longitude?: number; longtitude?: number }): number | undefined => {
    ...
    return (
        <>
            ...
            <div className="relative p-4 rounded-lg shadow-md bg-container w-full">
                ...
                <MapContainer center={[10.654, -61.501]} zoom={10} className="z-10 mt-2 h-48 w-full rounded-lg">
                    <TileLayer
            ...
            {isModalOpen && (
                <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 backdrop-blur-md z-[1000]">
                    <div ref={modalRef} className="relative bg-white rounded-lg shadow-lg w-[90vw] max-w-5xl p-4">
                        <button
                        ...
                        <MapContainer center={[10.654, -61.501]} zoom={10} className="h-[80vh] w-full rounded-lg">
                            <TileLayer
                                url="https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png"
                                attribution='&copy <a href="https://www.openstreetmap.org/copyright">OpenStreetMap</a>'
                            />
                            ...
                            {gamingLocations.map((location) => {
                                if (!location.geoCoords) return null;
                                return renderMarker(location.geoCoords.latitude, location.geoCoords, location.name, `modal-${location._id}`);
                            })}
                        </MapContainer>
                    </div>
                </div>
