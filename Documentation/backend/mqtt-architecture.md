...
This document provides a comprehensive architectural overview of the MQTT system implementation in the CMS (Casino Management System). It covers the complete system architecture, component interactions, data flow, and implementation details for developers.
...
Frontend → API → MQTT Service → MQTT Broker → SMIB Device
    ...
    │         │        │            │            │
    ▼         ▼        ▼            ▼            ▼
1. Request  2. Route  3. Publish  4. Forward  5. Process
   Config     to        to MQTT     to SMIB     Request
   via API    Service   Broker      Device
```
...
SMIB Device → MQTT Broker → MQTT Service → SSE Endpoint → Frontend
     ...
     │            │             │             │            │
     ▼            ▼             ▼             ▼            ▼
1. Send       2. Route      3. Route to    4. Forward   5. Update
...
Frontend → API → MQTT Service → MQTT Broker → SMIB Device
    ...
    │         │        │            │            │
    ▼         ▼        ▼            ▼            ▼
1. Update  2. Route  3. Publish  4. Forward  5. Apply
