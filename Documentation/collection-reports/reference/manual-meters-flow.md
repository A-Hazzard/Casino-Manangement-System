...
**meterSource enum**: `'COLLECTION_REPORT' | 'SAS_READ' | 'WOW_SYNC' | 'OTHER'`
- `COLLECTION_REPORT` — created manually during collection report creation/editing (includes supplemental meters for offline SMIB machines)
- `SAS_READ` — created by live SAS relay polling (movement deltas pre-computed)
- `WOW_SYNC` — created by the WOW sync system (cumulative absolute values, movement always 0)
- `OTHER` — fallback for unknown or legacy sources
...
## 7.5. WOW_SYNC Meter Source

WOW-synced machines produce `Meters` documents with a distinct structure from SAS_READ records:
...
```typescript
{
  ...
  isSupplemental: false,
  isRamClear: false,
  // Top-level fields are CUMULATIVE absolute values
  drop: 4101,                       // cumulative drop at reading time
  totalCancelledCredits: 401,       // cumulative TCC at reading time
  // movement fields are ALWAYS 0 — WOW sync does not pre-compute deltas
  movement: {
    drop: 0,
...
### WOW_SYNC vs SAS_READ

| Feature | SAS_READ | WOW_SYNC |
|---------|----------|----------|
| `movement.drop` | Per-reading delta (populated) | 0 (always) |
| Top-level `drop` | Per-reading absolute value | Cumulative absolute value |
| Variation pipeline | Sum `movement.*` | `lastDrop - firstDrop` fallback |
| `calculateSasMetrics` | Sum `movement.*` | `last.drop - first.drop` fallback |
| Source | Live SMIB relay polling | WOW sync database |

### WOW_SYNC Detection in Code

In `aggregateMeterDataForWindows` (variation.ts):
```typescript
anyWowSync: {
  $max: {
...
// Post-group: if anyWowSync && totalDrop === 0, use (lastDrop - firstDrop)
```

In `calculateSasMetrics` (creation.ts):
```typescript
const hasWowSync = metersInPeriod.some(meter => meter.meterSource === 'WOW_SYNC');
if (hasWowSync && metersInPeriod.length >= 2) {
  if (drop === 0) drop = (last.drop ?? 0) - (first.drop ?? 0);
}
```

The fix was applied June 2026 — before it, WOW_SYNC machines always showed $0 SAS gross, making their full machine gross appear as phantom variation.
