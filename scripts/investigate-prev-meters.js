...
async function investigatePrevMeters() {
  ...
  try {
    ...
    if (machine) {
      ...
      if (machine.collectionMetersHistory && machine.collectionMetersHistory.length > 0) {
        ...
        machine.collectionMetersHistory.forEach((entry, index) => {
          console.log(`   [${index}] ${entry.timestamp}: In=${entry.metersIn}, Out=${entry.metersOut}, PrevIn=${entry.prevMetersIn}, PrevOut=${entry.prevMetersOut}, ReportID=${entry.locationReportId}`);
        });
      }
    } else {
    ...
    console.log(`📋 Found ${allCollections.length} collections for machine ${machineId}:`);
    ...
    const collectionTimeForComparison = currentCollection.collectionTime || currentCollection.timestamp;
    
    console.log(`🔍 Looking for previous collection before: ${collectionTimeForComparison}`);
    ...
    if (previousCollection) {
      ...
      console.log(`   Location Report ID: ${previousCollection.locationReportId}`);
      ...
      const hasMismatch = currentCollection.prevIn !== expectedPrevIn || 
      ...
      if (hasMismatch) {
        console.log('\n⚠️  MISMATCH ANALYSIS:');
        console.log(`   Prev In Mismatch: ${currentCollection.prevIn} !== ${expectedPrevIn} (diff: ${expectedPrevIn - currentCollection.prevIn})`);
        console.log(`   Prev Out Mismatch: ${currentCollection.prevOut} !== ${expectedPrevOut} (diff: ${expectedPrevOut - currentCollection.prevOut})`);
        
        // Check if the mismatch is significant
        const prevInDiff = Math.abs(expectedPrevIn - currentCollection.prevIn);
        const prevOutDiff = Math.abs(expectedPrevOut - currentCollection.prevOut);
        ...
        if (prevInDiff > 1000000 || prevOutDiff > 1000000) {
          console.log('   🚨 LARGE MISMATCH DETECTED - This could cause significant calculation errors!');
        }
      }
      
    } else {
      console.log('❌ NO PREVIOUS COLLECTION FOUND');
      console.log('   This means the current collection should have prevIn=0, prevOut=0');
    ...
    if (incompleteCollections.length > 0) {
      console.log(`⚠️  Found ${incompleteCollections.length} incomplete collections that might interfere:`);
      incompleteCollections.forEach((col, index) => {
        console.log(`   [${index + 1}] ${col.timestamp}: In=${col.metersIn}, Out=${col.metersOut}, Completed=${col.isCompleted}`);
      });
    }
    
    // Check for collections with same timestamp (potential duplicates)
    const duplicateTimeCollections = await collections.find({
      machineId: machineId,
      $or: [
    ...
    if (duplicateTimeCollections.length > 1) {
      console.log(`⚠️  Found ${duplicateTimeCollections.length} collections with same timestamp (potential duplicates):`);
      duplicateTimeCollections.forEach((col, index) => {
        console.log(`   [${index + 1}] ${col._id}: ${col.timestamp}, Completed=${col.isCompleted}`);
      });
    }
    ...
    if (previousCollection) {
      ...
      if (hasMismatch) {
        console.log('✅ DETECTION LOGIC IS CORRECT - The mismatch is real and needs to be fixed');
        console.log('   The system correctly identified that prevIn/prevOut should be updated');
        console.log('   This is not a false positive - the data is actually inconsistent');
      } else {
        console.log('✅ DETECTION LOGIC IS CORRECT - Values match expected');
      }
    } else {
      if (currentCollection.prevIn === 0 && currentCollection.prevOut === 0) {
        console.log('✅ DETECTION LOGIC IS CORRECT - No previous collection, values should be 0');
        console.log('   This might be a false positive if the detection is still showing');
      } else {
        console.log('❓ DETECTION LOGIC NEEDS INVESTIGATION - Unexpected scenario');
      }
    }
