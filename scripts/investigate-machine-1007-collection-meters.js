...
async function investigateMachine1007CollectionMeters() {
  ...
  try {
    ...
    console.log(
      `📋 Found ${allCollections.length} collections for machine ${machine._id}:`
    );
    ...
    if (allCollections.length > 0) {
      ...
      console.log(
        `   Location Report ID: ${mostRecentCollection.locationReportId}`
      );
      ...
      console.log(
        `   Collection Current Meters In: ${mostRecentCollection.metersIn}`
      );
      console.log(
        `   Machine Collection Meters Out: ${machine.collectionMeters?.metersOut}`
      );
      console.log(
        `   Collection Current Meters Out: ${mostRecentCollection.metersOut}`
      );

      const metersInMatch =
        machine.collectionMeters?.metersIn === mostRecentCollection.metersIn;
      const metersOutMatch =
        machine.collectionMeters?.metersOut === mostRecentCollection.metersOut;

      console.log(`\n📊 MISMATCH ANALYSIS:`);
      console.log(`   Meters In Match: ${metersInMatch ? '✅ YES' : '❌ NO'}`);
      console.log(
        `   Meters Out Match: ${metersOutMatch ? '✅ YES' : '❌ NO'}`
      );
      ...
      if (!metersInMatch || !metersOutMatch) {
        ...
        if (!metersInMatch) {
          console.log(
            `   Meters In: Machine(${machine.collectionMeters?.metersIn}) !== Collection(${mostRecentCollection.metersIn})`
          );
        }
        if (!metersOutMatch) {
          console.log(
            `   Meters Out: Machine(${machine.collectionMeters?.metersOut}) !== Collection(${mostRecentCollection.metersOut})`
          );
        }
      }
      ...
      if (machineForCheck) {
        ...
        const machineMetersOut =
          machineForCheck.collectionMeters?.metersOut || 0;

        console.log(`   UI Check - Machine Meters In: ${machineMetersIn}`);
        console.log(`   UI Check - Machine Meters Out: ${machineMetersOut}`);
        console.log(
          `   UI Check - Collection Meters In: ${mostRecentCollection.metersIn}`
        );
        console.log(
          `   UI Check - Collection Meters Out: ${mostRecentCollection.metersOut}`
        );

        const uiDetectsMismatch =
          machineMetersIn !== mostRecentCollection.metersIn ||
          machineMetersOut !== mostRecentCollection.metersOut;

        console.log(
          `   UI Should Detect Mismatch: ${uiDetectsMismatch ? '✅ YES' : '❌ NO'}`
        );
      ...
      console.log(
        `   Current Machine Meters In: ${machine.collectionMeters?.metersIn}`
      );
      console.log(
        `   Current Machine Meters Out: ${machine.collectionMeters?.metersOut}`
      );

      const needsFix =
        machine.collectionMeters?.metersIn !== expectedMetersIn ||
      ...
      if (needsFix) {
        ...
        console.log(
          `      Meters In: ${machine.collectionMeters?.metersIn} → ${expectedMetersIn}`
        );
        console.log(
          `      Meters Out: ${machine.collectionMeters?.metersOut} → ${expectedMetersOut}`
        );
      }
    }
    ...
    Object.entries(timestampGroups).forEach(([timestamp, cols]) => {
      if (cols.length > 1) {
        console.log(
          `   ⚠️  Found ${cols.length} collections with same timestamp ${timestamp}:`
        );
        cols.forEach((col, index) => {
          console.log(
            `      [${index + 1}] ${col._id}: In=${col.metersIn}, Out=${col.metersOut}, Completed=${col.isCompleted}`
          );
        });
      }
    });
    ...
    if (allCollections.length > 0) {
      ...
      if (
        machineMetersIn !== mostRecent.metersIn ||
        machineMetersOut !== mostRecent.metersOut
      ) {
        console.log('🚨 ISSUES FOUND:');
        console.log(
          '   1. Machine collectionMeters do not match the most recent collection'
        );
        console.log(
          '   2. This should be detected by the UI but apparently is not'
        );
        console.log(
          '   3. The fix button should update the machine collectionMeters'
        );
        console.log('');
        console.log('🔧 SUGGESTED ACTIONS:');
        console.log(
          '   1. Check if the fix button is actually calling the fix-report API'
        );
        console.log(
          '   2. Check if the fix-report API is updating the machine collectionMeters'
        );
        console.log(
          '   3. Check if the UI detection logic is working correctly'
        );
        console.log(
          '   4. Manually update the machine collectionMeters if needed'
        );
      } else {
        console.log(
          '✅ NO ISSUES FOUND - Machine collectionMeters match the most recent collection'
        );
      }
    }
  } catch (error) {
