...
async function run() {
  try {
    ...
    for (const machine of machines) {
      console.log('\n=============================================');
      console.log(
        `Machine: ${machine.customName || machine.name} (ID: ${machine._id})`
      );
      ...
      if (machine.gamingLocation) {
        ...
        console.log(
          `Location Name: ${location?.name} (No SMIB Location? ${location?.noSMIBLocation})`
        );
      }

      // Check SMIB Config
      const smib = await db
        .collection('smibconfigs')
        .findOne({ relayId: machine.relayId });
      console.log(
        `SMIB Config exists: ${!!smib}, Location: ${smib?.locationId}`
      );
      ...
      // Check latest meters
      const latestMeters = await db
        .collection('meters')
        .find({ machine: machine._id.toString() })
        .sort({ timestamp: -1 })
        .limit(1)
        .toArray();
      ...
      if (latestMeters.length > 0) {
        ...
        console.log(
          `Latest Meter totalIn: ${latestMeters[0].totalIn}, totalOut: ${latestMeters[0].totalOut}`
        );
      } else {
      ...
      console.log(
        `Found ${sessions.length} recent ReportedMachine (V2) records for this machine:`
      );
      for (const sm of sessions) {
        console.log(`  Session ${sm.sessionId} [${sm.sessionStatus}]`);
        console.log(
          `    - sasMetersIn: ${sm.sasMetersIn}, sasMetersOut: ${sm.sasMetersOut}`
        );
        console.log(
          `    - manualMetersIn: ${sm.manualMetersIn}, manualMetersOut: ${sm.manualMetersOut}`
        );
