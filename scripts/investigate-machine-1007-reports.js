  ...
  try {
    ...
    Object.keys(collectionsByDate)
      ...
      .forEach(date => {
        ...
        dayCollections.forEach((collection, index) => {
          ...
          console.log(
            `     Meters In: ${collection.metersIn}, Meters Out: ${collection.metersOut}`
          );
          console.log(
            `     Prev In: ${collection.prevIn}, Prev Out: ${collection.prevOut}`
          );
          console.log(
            `     Movement In: ${collection.movement?.metersIn}, Movement Out: ${collection.movement?.metersOut}`
          );
          console.log(
            `     Location Report ID: ${collection.locationReportId}`
          );
      ...
      Object.keys(reportsByDate)
        ...
        .forEach(date => {
          ...
          dayReports.forEach((report, index) => {
            console.log(
              `  ${index + 1}. Report ID: ${report.locationReportId}`
            );
            console.log(`     Timestamp: ${report.timestamp}`);
            console.log(`     Collector: ${report.collectorName}`);
            console.log(
              `     Total Collections: ${report.totalCollections || 'N/A'}`
            );
            console.log('');
          });
        });
    ...
    for (let i = 0; i < collections.length; i++) {
      ...
      const prevCollection =
        i < collections.length - 1 ? collections[i + 1] : null;

      console.log(
        `\nCollection ${collection._id} (${new Date(collection.timestamp).toDateString()}):`
      );
      console.log(
        `  Current: Meters In: ${collection.metersIn}, Meters Out: ${collection.metersOut}`
      );
      console.log(
        `  Prev: Prev In: ${collection.prevIn}, Prev Out: ${collection.prevOut}`
      );
      ...
      if (prevCollection) {
        ...
        console.log(
          `  Expected Prev: ${expectedPrevIn}, ${expectedPrevOut} (from previous collection)`
        );
        ...
        if (collection.prevIn !== 0 || collection.prevOut !== 0) {
          console.log(
            `  ❌ ISSUE: First collection should have prevIn=0, prevOut=0`
          );
          issuesFound++;
        }
      }
    ...
    console.log(
      `- Machine collectionMeters: ${machine.collectionMeters?.metersIn}, ${machine.collectionMeters?.metersOut}`
    );
