...
async function investigateMachine1007Detailed() {
  ...
  try {
    ...
    console.log(`      Meters In: ${mostRecentCollection.metersIn} (current collection)`);
    console.log(`      Meters Out: ${mostRecentCollection.metersOut} (current collection)`);
    console.log('   ');
    console.log('   OPTION B: Previous collection meters (what was fixed):');
    console.log(`      Meters In: ${mostRecentCollection.prevIn} (previous collection)`);
    console.log(`      Meters Out: ${mostRecentCollection.prevOut} (previous collection)`);
    console.log('   ');
    ...
    const matchesCurrent = machineMetersIn === mostRecentCollection.metersIn && 
                          machineMetersOut === mostRecentCollection.metersOut;
    const matchesPrevious = machineMetersIn === mostRecentCollection.prevIn && 
                           machineMetersOut === mostRecentCollection.prevOut;
    
    console.log(`   Machine matches CURRENT collection meters: ${matchesCurrent ? '✅ YES' : '❌ NO'}`);
    console.log(`   Machine matches PREVIOUS collection meters: ${matchesPrevious ? '✅ YES' : '❌ NO'}`);
    
    if (matchesCurrent) {
      console.log('   📋 CONCLUSION: Machine collectionMeters contain the CURRENT collection meters');
      console.log('   📋 This is the standard behavior - machine meters should reflect the current state');
    } else if (matchesPrevious) {
      console.log('   📋 CONCLUSION: Machine collectionMeters contain the PREVIOUS collection meters');
      console.log('   📋 This would be unusual - machine meters should reflect current state');
    } else {
      console.log('   📋 CONCLUSION: Machine collectionMeters match neither current nor previous');
    ...
    console.log(`      Collection prevOut: 0 → ${mostRecentCollection.prevOut}`);
    console.log('   ');
    console.log('   The fix button should also update machine collectionMeters to:');
    console.log(`      Machine metersIn: ${mostRecentCollection.metersIn} (current collection meters)`);
    console.log(`      Machine metersOut: ${mostRecentCollection.metersOut} (current collection meters)`);
    console.log('   ');
    console.log('   This ensures that:');
    console.log('   1. Collection has correct prevIn/prevOut (previous collection meters)');
    console.log('   2. Machine has current collection meters (for next collection)');
    
    console.log('\n6️⃣ VERIFICATION:');
    if (matchesCurrent) {
      console.log('   ✅ VERIFIED: Machine collectionMeters correctly contain current collection meters');
      console.log('   ✅ The fix button worked correctly');
      console.log('   ✅ No further action needed');
    } else {
      console.log('   ❌ ISSUE: Machine collectionMeters do not contain current collection meters');
      console.log('   🔧 Manual fix needed: Update machine collectionMeters to match current collection');
    }
    
    console.log('\n7️⃣ NEXT COLLECTION SCENARIO:');
    console.log('   When creating the next collection for this machine:');
    console.log(`   - prevIn should be: ${mostRecentCollection.metersIn} (current collection meters)`);
    console.log(`   - prevOut should be: ${mostRecentCollection.metersOut} (current collection meters)`);
    console.log(`   - These values should come from: machine.collectionMeters`);
    console.log(`   - Current machine.collectionMeters: ${machineMetersIn}, ${machineMetersOut}`);
    
    if (machineMetersIn === mostRecentCollection.metersIn && machineMetersOut === mostRecentCollection.metersOut) {
