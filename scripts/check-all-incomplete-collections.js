...
async function checkAllIncompleteCollections() {
  ...
  try {
    ...
    console.log('📊 Total Incomplete Collections:', incompleteCollections.length);
    ...
    const collectionsWithoutReport = await collections.find({
      isCompleted: true,
      $or: [
    ...
    console.log('\n📊 Collections without locationReportId:', collectionsWithoutReport.length);
    if (collectionsWithoutReport.length > 0) {
      collectionsWithoutReport.forEach((col, index) => {
        console.log(`${index + 1}. ID: ${col._id}, Machine: ${col.machineName}, Location: ${col.location}`);
      });
    }
