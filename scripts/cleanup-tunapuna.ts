...
async function runCleanup() {
  ...
  try {
    ...
    const locResult = await db.collection<TypedDocument>('gaminglocations').deleteMany({
      _id: { $ne: TARGET_LOCATION_ID }
    });
    console.log(`✅ Deleted ${locResult.deletedCount} other locations from 'gaminglocations'.`);

    // 2. Machines Cleanup
    const machResult = await db.collection<{ gamingLocation: string } & Record<string, unknown>>('machines').deleteMany({
      gamingLocation: { $ne: TARGET_LOCATION_ID }
    });
    console.log(`✅ Deleted ${machResult.deletedCount} machines not belonging to target from 'machines'.`);

    // 3. Meters Cleanup
    const meterResult = await db.collection<{ location: string } & Record<string, unknown>>('meters').deleteMany({
      location: { $ne: TARGET_LOCATION_ID }
    });
    console.log(`✅ Deleted ${meterResult.deletedCount} meter records not belonging to target from 'meters'.`);

    // 4. Users Cleanup
    // Delete users who are NOT assigned to this location
    const userResult = await db.collection<{ assignedLocations: string[] } & Record<string, unknown>>('users').deleteMany({
      assignedLocations: { $ne: TARGET_LOCATION_ID }
    });
    console.log(`✅ Deleted ${userResult.deletedCount} users not assigned to target from 'users'.`);
    ...
    for (const coll of collectionsToCleanup) {
      try {
        ...
        const result = await db.collection<Record<string, unknown>>(coll.name).deleteMany(filter);
        console.log(`✅ Deleted ${result.deletedCount} records from '${coll.name}'.`);
      } catch (e) {
        console.warn(`⚠️ Could not clean ${coll.name}: ${e instanceof Error ? e.message : 'Unknown error'}`);
      }
    }

    console.log('\n✨ --- CLEANUP COMPLETE ---');
  } catch (err) {
    console.error('\n❌ CRITICAL ERROR DURING CLEANUP:', err instanceof Error ? err.message : 'Unknown error');
  } finally {
    await client.close();
  }
}
