...
async function checkMachineId() {
  try {
    ...
    for (const collection of collections) {
      ...
      console.log(
        `Machine Name: ${collection.machineName || collection.machineCustomName}`
      );
      ...
      if (machine) {
        ...
        console.log(
          `History entries: ${machine.collectionMetersHistory?.length || 0}`
        );
      } else {
        console.log(`❌ Machine not found with ObjectId`);

        // Try as string
        const machine2 = await mongoose.connection.db
          .collection('machines')
          .findOne({
        ...
        if (machine2) {
          ...
          console.log(
            `History entries: ${machine2.collectionMetersHistory?.length || 0}`
          );
        } else {
