...
async function testDetectionFinal() {
  try {
    ...
    // Find the report
    const report = await mongoose.connection.db.collection('collectionreports').findOne({
    ...
    for (const machineId of machineIds) {
      let machine = await mongoose.connection.db.collection('machines').findOne({
      ...
      if (machine) {
        ...
        for (let i = 0; i < history.length; i++) {
          ...
          if (seenEntries.has(key)) {
            ...
            console.log(`❌ DUPLICATE: Machine ${machine.serialNumber} has duplicate history entry: ${key}`);
            break;
          }
        ...
        if (!hasMachineHistoryIssues) {
          for (let i = 1; i < history.length; i++) {
            ...
            if (prevInDiff > 0.1 || prevOutDiff > 0.1) {
              hasMachineHistoryIssues = true;
              console.log(`❌ PREV METERS MISMATCH: Machine ${machine.serialNumber} entry ${i}: expected ${expectedPrevIn}/${expectedPrevOut}, got ${actualPrevIn}/${actualPrevOut}`);
              break;
            }
          }
        }
    ...
    console.log(`\n📊 Final Results:`);
    console.log(`Report ${reportId} has ${reportIssues[report._id.toString()].issueCount} issues`);
