...
description: Logging in and manually testing the Evolution One CMS in a real browser on localhost. Use when driving the app via the browser (Codex in Chrome) to verify a change, test a flow, or reproduce a bug. Contains the dev login credentials.
---

# Local App Testing

Use this when testing the running app in a real browser (Codex in Chrome) against the local dev server.
...
- This is a **real browser** flow (Codex in Chrome MCP), not the Codex Preview tools.
