...
export async function getAllCollectionReportsWithMachineCounts(
  ...
  if (!licenceeId) {
    ...
    console.warn(
      `[COLLECTION REPORTS] Running aggregation with licensee: ${licenceeId}`
    );
    console.warn(
      `[COLLECTION REPORTS] Match criteria:`,
      JSON.stringify(matchCriteria, null, 2)
    );
    console.warn(
      `[COLLECTION REPORTS] Aggregation pipeline:`,
    ...
    try {
      ...
      console.warn(
        `[COLLECTION REPORTS] Raw reports found: ${rawReports.length}`
      );
    } catch (error) {
  ...
  console.warn(
    `[COLLECTION REPORTS] Starting enrichment process for ${rawReports.length} raw reports`
  );
  ...
  try {
    enrichedReports = await Promise.all(
      rawReports.map(async (doc: Record<string, unknown>) => {
        ...
        console.warn(
          `[COLLECTION REPORTS] Processing report: ${locationName} (${locationReportId})`
        );
        ...
        console.warn(
          `[COLLECTION REPORTS] Completed processing report: ${locationName} - Machines: ${collectedMachines}/${totalMachines}`
        );
    ...
    console.warn(
      `[COLLECTION REPORTS] Enrichment complete. Returning ${enrichedReports.length} enriched reports`
    );
  } catch (error) {
