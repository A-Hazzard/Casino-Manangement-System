...
import { MachineStats } from '@/shared/types/machines';
 ...
 * Uses the new dedicated /api/cabinets/status endpoint that properly filters
...
export async function fetchMachineStats(
  ...
  try {
    ...
    const res = await axios.get(`/api/cabinets/status?${params.toString()}`, {
      signal,
    });
    const data = res.data;
