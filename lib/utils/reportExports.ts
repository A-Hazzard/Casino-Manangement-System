...
export class ReportExportUtils {
   ...
   */
  static exportActiveCustomersToCSV(
    data: ActiveCustomersReport,
    filename?: string
  ) {
    const csvRows = [];
   ...
   */
  static exportLocationStatsToCSV(
    data: LocationStatsReport[],
    filename?: string
  ) {
    if (!data || data.length === 0) {
      throw new Error('No data to export');
   ...
   */
  static exportMachinePerformanceToCSV(
    data: MachinePerformanceReport[],
    filename?: string
  ) {
    if (!data || data.length === 0) {
      throw new Error('No data to export');
   ...
   */
  static exportTerminalCountsToCSV(
    data: TerminalCountsReport[],
    filename?: string
  ) {
    if (!data || data.length === 0) {
      throw new Error('No data to export');
