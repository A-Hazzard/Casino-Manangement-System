...
export function useSafeGSAPAnimation() {
    ...
    const executeAnimation = () => {
      try {
        ...
        if (typeof selector === 'string') {
          ...
          targets = selector.filter(el => el && el instanceof Element && el.isConnected);
        ...
        if (validTargets.length === 0) {
          console.warn(`useSafeGSAPAnimation: No valid targets found for selector: ${selector}`);
          return;
        }
  ...
  const animateCards = useCallback((
    ...
    const { stagger = 0.05, duration = 0.3, ease = "back.out(1.5)" } = options;
