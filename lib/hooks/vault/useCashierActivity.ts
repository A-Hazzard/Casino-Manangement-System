        ...
        if (shiftsRes.ok && requestsRes.ok && payoutsRes.ok) {
          ...
          (shiftsData.shifts || []).forEach((shift: CashierShift) => {
            ...
            if (shift.status === 'pending_start')
              actionLabel = 'Opening Shift Request';
              ...
              actionLabel = 'Shift Pending Review';
          ...
          (requestsData.data || []).forEach((floatRequest: FloatRequest) => {
            ...
            const isInitial =
              floatRequest.requestNotes === 'Initial shift float';

            combined.push({
              id: floatRequest._id,
              type: 'float_request',
              action: isInitial
                ? 'Initial Float Request'
                : `Float ${floatRequest.type === 'increase' ? 'Increase' : 'Decrease'} Request`,
              ...
              details:
                floatRequest.status === 'denied'
                  ? `Reason: ${floatRequest.vmNotes || ''}`
                  : undefined,
          ...
          (payoutsData.payouts || []).forEach((payout: PayoutItem) => {
            combined.push({
              ...
              type: 'payout',
              action:
                payout.type === 'ticket' ? `Ticket Redemption` : `Hand Pay`,
              ...
              notes:
                payout.type === 'ticket'
                  ...
                  : `Machine: ${payout.machineSerialNumber || payout.machineId}${payout.notes ? ` - ${payout.notes}` : ''}`,
              isOutflow: true,
            });
          });
