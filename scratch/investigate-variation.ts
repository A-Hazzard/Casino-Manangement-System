 ...
 *
 * Usage:
 *   bun run scratch/investigate-variation.ts <locationReportId>
 *   bun run scratch/investigate-variation.ts          ← most recent report
 */
...
async function main() {
  ...
  // ──────────────────────────────────────────────────────────────────────────
  // STEP 3: Collections for this report
  // ──────────────────────────────────────────────────────────────────────────
  ...
  for (const col of colls) {
    ...
    row('sasMeters.gross (stored snapshot):', fmt(col.sasMeters?.gross));
    ...
    // Partition into inside / before / after the SAS window
    ...
    // ── Search 1: SMIB-only (exclude COLLECTION_REPORT) ──────────────────
    ...
    // ── Search 2: All meters, but MUST include the CR meter (if any) ──────
    // This finds the window that works when CR meters are intentional and
    // the user just has the boundaries wrong.
    const matchWithCR = crMetersInWindow.length > 0
      ? findMatchingWindow(wideMeters, dropDelta, cancelDelta, crMetersInWindow[0].readAt)
      : null;

    // Print SMIB-only result
    if (matchSmib?.exact) {
    ...
    // ── Check for rogue absolute-value meters ─────────────────────────────
    ...
    // Must be strictly BEFORE this report's timestamp so we don't pick up later reports
    ...
    const prevCol = await db.collection('collections').findOne(
      {
        machineId: mid,
        isCompleted: true,
        locationReportId: { $exists: true, $ne: String(report.locationReportId) },
    ...
    computedTotalGross += machineGross;
    computedTotalSasGross += currentWindowSum.gross;
    computedTotalVariation += currentVariation;
  }

  // ──────────────────────────────────────────────────────────────────────────
  // STEP 4: Report-level summary
  // ──────────────────────────────────────────────────────────────────────────
