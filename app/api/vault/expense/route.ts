    ...
    try {
      ...
      if (denominationsStr) {
        try {
          ...
          console.error('Denom parse error');
        }
      }
      ...
      if (
        allowedLocs !== 'all' &&
        !allowedLocs.includes(String(activeVaultShift.locationId))
      ) {
        logRouteError(
          functionName,
          'POST',
      ...
      if (
        ...
        !validateDenominationTotal(amount, denominations)
      ) {
        logRouteError(
          functionName,
          'POST',
        ...
        return NextResponse.json(
          { success: false, error: 'Invalid denominations' },
          { status: 400 }
        );
      }
      ...
      if (
        ...
        const mList = await Machine.find({
          _id: { $in: expenseDetails.machineIds },
        }).lean<GamingMachine[]>();
        expenseDetails.machineDetails = expenseDetails.machineIds.map(
          (id: string) => {
            const machineItem = mList.find(
              (x: Record<string, unknown>) =>
                String(x._id) === id ||
                String(x.machineId) === id ||
                (x.serialNumber as string) === id
            ) as Record<string, unknown> | undefined;
            if (!machineItem)
              return { identifier: id, game: 'N/A', gameType: 'N/A' };
            const custom = machineItem.custom as { name?: string } | undefined;
            const mainId =
              ((machineItem.serialNumber as string) || '')?.trim() ||
              custom?.name?.trim() ||
              'N/A';
            return {
              identifier: custom?.name ? `${mainId} (${custom.name})` : mainId,
              game: (
                ((machineItem.game as string) ||
                  (machineItem.installedGame as string) ||
                  '') as string
              ).trim(),
              gameType: ((machineItem.gameType as string) || '').trim(),
            };
          }
        );
      }
...
export async function GET(request: NextRequest) {
  ...
  return withApiAuth(request, async ({ user: userPayload, userRoles }) => {
    try {
      ...
      if (locId) {
        if (allowedLocs !== 'all' && !allowedLocs.includes(locId)) {
          logRouteError(
            functionName,
            'GET',
            '/api/vault/expense',
      ...
      expenses.forEach((expenseItem: LocalTransaction) => {
        if (
          expenseItem.expenseDetails?.isMachineRepair &&
          expenseItem.expenseDetails.machineIds?.length &&
          !expenseItem.expenseDetails.machineDetails?.length
        ) {
          expenseItem.expenseDetails.machineIds.forEach((id: string) =>
            missingIds.add(id)
          );
        }
      });
      ...
      if (missingIds.size > 0) {
        ...
        expenses.forEach((expense: LocalTransaction) => {
          if (
            expense.expenseDetails?.isMachineRepair &&
            expense.expenseDetails.machineIds?.length &&
            !expense.expenseDetails.machineDetails?.length
          ) {
            expense.expenseDetails.machineDetails =
              expense.expenseDetails.machineIds.map((id: string) => {
                const machineItem = mList.find(
                  (x: GamingMachine) =>
                    String(x._id) === id ||
                    String(x.machineId) === id ||
                ...
                if (!machineItem)
                  return { identifier: id, game: 'N/A', gameType: 'N/A' };
                const custom = machineItem.custom as
                  | { name?: string }
                  | undefined;
                const mainId =
                  ((machineItem.serialNumber as string) || '')?.trim() ||
