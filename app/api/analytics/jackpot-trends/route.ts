...
export async function GET(req: NextRequest) {
  try {
    ...
    const pipeline = [
      ...
      // Group by time period
      {
        $group: {
          _id: {
            $dateToString: {
              format: timePeriod === "Today" || timePeriod === "Yesterday" ? "%H:00" : "%Y-%m-%d",
              date: "$readAt"
            }
          },
    ...
    const jackpotTrends = await db.collection("meters").aggregate(pipeline).toArray();
    ...
    console.error("Error fetching jackpot trends:", error);
    return NextResponse.json(
