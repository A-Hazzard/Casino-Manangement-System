...
export async function GET(req: NextRequest) {
  try {
    ...
    const targetLocations = locationIds ? locationIds.split(",").map(id => id.trim()) : [];
    const targetMachines = machineIds ? machineIds.split(",").map(id => id.trim()) : [];
    ...
    const pipeline = [
      {
        $match: {
          ...
          ...(targetLocations.length > 0 && { location: { $in: targetLocations } }),
          ...(targetMachines.length > 0 && { machine: { $in: targetMachines } }),
        },
      },
      {
        $lookup: {
    ...
    const hourlyTrends = Array.from({ length: 24 }, (_, hour) => {
      ...
      Object.keys(locationHourlyData).forEach(locationKey => {
        const locationHourData = hourData.filter(item => 
          item.location === locationKey
        );
        
        // Sum up all machines for this location in this hour
        const totalHandle = locationHourData.reduce((sum, item) => sum + item.handle, 0);
        const totalWinLoss = locationHourData.reduce((sum, item) => sum + item.winLoss, 0);
        const totalJackpot = locationHourData.reduce((sum, item) => sum + item.jackpot, 0);
        const totalPlays = locationHourData.reduce((sum, item) => sum + item.plays, 0);
    ...
    const totals = hourlyTrends.reduce(
      (acc, item) => {
        Object.keys(item).forEach(key => {
          if (key !== 'hour' && typeof item[key] === 'object') {
            if (!acc[key]) acc[key] = { handle: 0, winLoss: 0, jackpot: 0, plays: 0 };
      ...
      {} as Record<string, { handle: number; winLoss: number; jackpot: number; plays: number }>
    );
