...
export async function GET(req: NextRequest) {
  return withApiAuth(
    ...
    async ({ user: userPayload, userRoles, isAdminOrDev }) => {
      ...
      const functionName = 'GET /api/reports/machines';
      ...
      try {
        ...
        // ============================================================================
        const moneyInMult =
          (userPayload as { moneyInMultiplier?: number | null })
            ?.moneyInMultiplier ?? null;
        const moneyOutMult =
          (userPayload as { moneyOutAndJackpotMultiplier?: number | null })
