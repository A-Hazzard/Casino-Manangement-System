  ...
  return withApiAuth(
    ...
    async ({ user: userPayload, userRoles, isAdminOrDev }) => {
      ...
      try {
        ...
        // ============================================================================
        console.log(
          '[Meters Report API] STEP 2: Determining accessible locations...'
        );
        ...
        if (params.includeHourlyData && locationList.length > 0) {
          const machineIdsForHourlyData =
            ...
            params.hourlyDataMachineIds.length > 0
              ? params.hourlyDataMachineIds
              : machineIds;
        ...
        // ============================================================================
        console.log(
          '[Meters Report API] STEP 9: Applying reviewer multiplier if present...'
        );
        const scaleReferenceDate = queryEndDate || queryStartDate || null;
        const scalableUser = userPayload as ReviewerScalableUser;
        const moneyInScale = getMoneyInScale(scalableUser, scaleReferenceDate);
        ...
        if (moneyInScale !== 1 || moneyOutScale !== 1) {
          transformedData = transformedData.map(item =>
            scaleMeterReportRow(item, moneyInScale, moneyOutScale)
          );
        }
        ...
        return NextResponse.json(
          { message: 'Failed to fetch meters report', error: errorMessage },
          { status: 500 }
        );
      }
    }
