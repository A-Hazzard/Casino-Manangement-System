...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const customStartDate = startDateParam ? new Date(startDateParam) : undefined;
    ...
    // ============================================================================
    const matchingLocations = await fetchMachineStats(locationMatch, showArchived);
    ...
    const {
      ...
      allLocationIds,
      globalEnd,
    } = await computeFinancialMetrics(matchingLocations, timePeriod, customStartDate, customEndDate);
    ...
    if (syncAll && allLocationIds.length > 0) {
      ...
      const locationsWithMachines = await fetchLocationsWithMachinesForSmib(allLocationIds);
    ...
    // ============================================================================
    const filteredLocations = applyOnlineFilterAndSort(hydratedLocations, onlineStatus);
