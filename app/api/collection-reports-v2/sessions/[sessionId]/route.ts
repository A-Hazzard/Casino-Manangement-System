    ...
    await logActivity({
      ...
      metadata: {
        ...
        previousData: {
          ...
          locationName,
          machineCount: count,
          sessionStartTime: sessionMachines[0]?.sessionStartTime?.toISOString?.() ?? null,
          sessionEndTime: sessionMachines[0]?.sessionEndTime?.toISOString?.() ?? null,
          machines: sessionMachines.map(machine => ({
            _id: machine._id,
            ...
            sasStartTime: machine.sasStartTime?.toISOString?.() ?? null,
            sasEndTime: machine.sasEndTime?.toISOString?.() ?? null,
            sasMetersIn: machine.sasMetersIn,
            sasMetersOut: machine.sasMetersOut,
            sasGross: machine.sasGross ?? null,
            prevSasMetersIn: machine.prevSasMetersIn ?? null,
            prevSasMetersOut: machine.prevSasMetersOut ?? null,
            metersMatch: machine.metersMatch,
            ramClear: machine.ramClear ?? false,
            ramClearMetersIn: machine.ramClearMetersIn ?? null,
            ramClearMetersOut: machine.ramClearMetersOut ?? null,
            manualMetersIn: machine.manualMetersIn ?? null,
            manualMetersOut: machine.manualMetersOut ?? null,
            movement: machine.movement ?? null,
            notes: machine.notes || null,
            status: machine.status,
            createdAt: machine.createdAt?.toISOString?.() ?? null,
          })),
        },
        changes: [
          { field: 'sessionId', oldValue: sessionId, newValue: null },
          { field: 'locationName', oldValue: locationName, newValue: null },
          { field: 'machineCount', oldValue: count, newValue: null },
          { field: 'sessionStartTime', oldValue: sessionMachines[0]?.sessionStartTime?.toISOString?.() ?? null, newValue: null },
          { field: 'sessionEndTime', oldValue: sessionMachines[0]?.sessionEndTime?.toISOString?.() ?? null, newValue: null },
          ...sessionMachines.map(machine => ({
            field: `machine_${machine.machineCustomName || machine.machineName || machine.machineId}_details`,
            oldValue: `${machine.machineCustomName || machine.machineName || machine.machineId}: SAS In: ${machine.sasMetersIn ?? 'N/A'}, SAS Out: ${machine.sasMetersOut ?? 'N/A'}${machine.prevSasMetersIn !== undefined ? ` (Prev: ${machine.prevSasMetersIn} In, ${machine.prevSasMetersOut} Out)` : ''}${machine.ramClear ? ', RAM Cleared' : ''}${machine.notes ? `, Notes: ${machine.notes}` : ''}`,
            newValue: null,
          })),
        ],
      },
