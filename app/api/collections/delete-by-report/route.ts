...
export async function DELETE(request: NextRequest) {
  try {
    ...
    console.warn("Collections details:", collections.map(c => ({
      _id: c._id,
    ...
    const machineIds = [...new Set(collections.map(c => c.machineId).filter(Boolean))];
    ...
    const machineUpdatePromises = collections.map(async (collection) => {
      try {
        ...
        console.error(`Failed to revert machine ${collection.machineId}:`, error);
      }
    });
    ...
    const reportDeleteResult = await CollectionReport.deleteOne({ locationReportId });
    ...
    for (const machineId of machineIds) {
      const machine = await Machine.findById(machineId).lean() as { collectionMetersHistory?: Array<{ locationReportId: string }> } | null;
