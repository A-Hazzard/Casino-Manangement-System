...
): Promise<{ correctPrevIn: number; correctPrevOut: number; wasFixed: boolean }> {
...
export async function fixCollectionSasTimes(
  ...
  const machineId = collection.machineId;
  if (!machineId) {
    return { success: false, error: 'No machine ID' };
  }

  try {
    // Fix prevIn/prevOut
    const { correctPrevIn, correctPrevOut, wasFixed: prevFixed } =
 ...
 */
export async function fixSasTimesForReport(
  reportId: string
): Promise<{
  success: boolean;
  ...
  for (const report of allReportsToProcess) {
    ...
    try {
      const machineIds = [
        ...new Set(
          reportCollections.map(c => c.machineId).filter(Boolean)
        ),
      ];
 ...
 */
export async function fixCollectionHistoryForReport(
  reportId: string
): Promise<{
  success: boolean;
