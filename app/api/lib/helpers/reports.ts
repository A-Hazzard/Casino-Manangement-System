    ...
    for (const fieldId of fields) {
      switch (fieldId) {
        ...
        case "machineManufacturer":
          ...
          break;
        case "totalHandle":
          row[fieldId] = machine.coinIn;
          break;
        case "totalWin":
          row[fieldId] = machine.coinIn - machine.coinOut;
          break;
        case "actualHold":
          row[fieldId] = machine.coinIn > 0 ? (machine.coinIn - machine.coinOut) / machine.coinIn : 0;
          break;
        case "gamesPlayed":
          row[fieldId] = machine.gamesPlayed;
          break;
        case "averageWager":
            row[fieldId] = machine.gamesPlayed > 0 ? machine.coinIn / machine.gamesPlayed : 0;
            break;
        case "jackpot":
  ...
  export function generateReportData(config: ReportConfig): ReportData {
    ...
    const summary = {
      ...
      keyMetrics: [
        ...
        {
          label: "Total Win",
          value: filteredMachines.reduce((sum, m) => sum + (m.coinIn - m.coinOut), 0),
        },
      ],
    };
