 ...
 */
export async function getAttributionDate(
  openedAt: Date,
  locationId: string
): Promise<Date> {
  if (
    ...
    console.error(
      '[getAttributionDate] openedAt (Date) and locationId (string) are required'
    );
    return new Date();
  }
 ...
 */
export async function isShiftStaleBackend(
  openedAt: Date,
  locationId: string
): Promise<boolean> {
  if (
    ...
    console.error(
      '[isShiftStaleBackend] openedAt (Date) and locationId (string) are required'
    );
    return false;
  }
