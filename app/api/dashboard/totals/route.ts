  ...
  try {
    ...
    if (isAdminOrDev && shouldApplyCurrencyConversion(licencee)) {
      ...
      if (userAccessibleLicensees !== 'all') {
        allLicenseeIds = allLicenseeIds.filter(
          id => id === null || userAccessibleLicensees.includes(id)
        );
      }
      ...
      console.log(
        `[DASHBOARD DEBUG] Processing ${allLicenseeIds.length} licensees in parallel for timePeriod="${timePeriod}"...`
      );
      console.log(
        `[DASHBOARD DEBUG] Licensee IDs:`,
        allLicenseeIds.slice(0, 3),
        '...'
      );
      ...
      const licenseeResults = await Promise.all(
        allLicenseeIds.map(async licenseeId => {
          ...
          if (gamingDayRanges.size > 0) {
            ...
            console.log(
              `[DASHBOARD DEBUG] Gaming day range for ${timePeriod}:`,
              {
          ...
          for (
            let i = 0;
            i < filteredLocationsWithOffset.length;
            i += BATCH_SIZE
          ) {
            ...
            const batchResults = await Promise.all(
              batch.map(async location => {
                ...
                const metricsResult = await (async () => {
                  // Get machine IDs first (exclude deleted machines)
                  const machines = await db
                    .collection('machines')
                    .find(
                      {
          ...
          const { licenseeMoneyIn, licenseeMoneyOut, licenseeGross } =
            locationResults.reduce(
              (acc, curr) => ({
                licenseeMoneyIn:
                  (acc.licenseeMoneyIn || 0) + (curr.moneyIn || 0),
                licenseeMoneyOut:
                  (acc.licenseeMoneyOut || 0) + (curr.moneyOut || 0),
          ...
          if (!licenseeId) {
            ...
            if (
              locationsWithOffset.length > 0 &&
              locationsWithOffset[0].country
            ) {
              const countryName = countryIdToName.get(
                locationsWithOffset[0].country.toString()
              );
      ...
      console.log(
        `[DASHBOARD] ⚡ Processed ${allLicenseeIds.length} licensees in parallel in ${parallelTime}ms`
      );
      ...
      if (licencee && licencee !== 'all') {
        ...
        const locationResults2: Array<{
          moneyIn: number;
          moneyOut: number;
          gross: number;
        }> = [];

        // Process accessible locations in parallel batches (FIX: use accessibleLocations, not locations)
        for (let i = 0; i < accessibleLocations.length; i += BATCH_SIZE_2) {
          const batch = accessibleLocations.slice(i, i + BATCH_SIZE_2);
        ...
        // Sum all location results
        const { totalMoneyIn, totalMoneyOut, totalGross } =
          locationResults2.reduce(
            (acc, curr) => ({
