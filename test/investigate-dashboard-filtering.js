...
async function investigate() {
  ...
  try {
    ...
    for (const licensee of licensees) {
      ...
      const locations = await db
        ...
        .find(
          ...
          { projection: { _id: 1, name: 1, gameDayOffset: 1 } }
        )
        .toArray();
    ...
    const unassignedLocations = await db
      .collection('gaminglocations')
      .find(
        {
          $or: [
    ...
    if (unassignedLocations.length > 0) {
      ...
      const unassignedMachines = await db
        .collection('machines')
        .find(
          {
      ...
      if (unassignedMachines.length > 0) {
        ...
        const unassignedAggregation = await db
          .collection('meters')
          .aggregate([
            {
              $match: {
    ...
    const total = licenseeResults.reduce(
      (sum, lic) => ({
    ...
    console.log(
      `   Licensees with data: ${licenseeResults.filter(l => l.moneyIn > 0).length}`
    );
    ...
    licenseeResults.forEach(lic => {
      if (lic.moneyIn > 0) {
        console.log(
          `   ${lic.licenseeName}: $${lic.moneyIn.toFixed(2)} (${lic.locationCount} locations, ${lic.machineCount} machines)`
        );
      }
    });
    ...
    console.log(
      `   Difference: $${Math.abs(rawMoneyIn - total.moneyIn).toFixed(2)}`
    );
    ...
    if (Math.abs(rawMoneyIn - total.moneyIn) > 0.01) {
      ...
      const allLocationIdsSet = new Set(
