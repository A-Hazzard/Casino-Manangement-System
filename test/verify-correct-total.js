...
async function main() {
  ...
  try {
    ...
    const allLocations = await db.collection('gaminglocations')
      .find({
        $or: [
          { deletedAt: null },
    ...
    console.log(`   Found ${allLocations.length} locations (after deletedAt filter)\n`);

    // ============================================================================
    // STEP 2: Get ALL locations WITHOUT deletedAt filtering
    // ============================================================================
    console.log('📍 Fetching ALL locations (WITHOUT deletedAt filter)...');
    const allLocationsIncludingDeleted = await db.collection('gaminglocations')
      .find({}, {
        projection: {
          _id: 1,
          name: 1,
    ...
    const deletedCount = allLocationsIncludingDeleted.length - allLocations.length;
    console.log(`   Found ${allLocationsIncludingDeleted.length} total locations (${deletedCount} deleted)\n`);
    ...
    console.log(`   Total WITH deletedAt filter: Money In = ${totalWithDeletedAtFilter.moneyIn.toFixed(2)}`);
    ...
    console.log(`   Total WITHOUT deletedAt filter: Money In = ${totalWithoutDeletedAtFilter.moneyIn.toFixed(2)}\n`);
    ...
    console.log(`WITH deletedAt filter (correct):    Money In = ${totalWithDeletedAtFilter.moneyIn.toFixed(2)}`);
    console.log(`WITHOUT deletedAt filter (wrong):   Money In = ${totalWithoutDeletedAtFilter.moneyIn.toFixed(2)}`);
    console.log(`Difference: ${(totalWithoutDeletedAtFilter.moneyIn - totalWithDeletedAtFilter.moneyIn).toFixed(2)}`);
    console.log('='.repeat(80));
    console.log();
    ...
    if (locationsWithDeletedAt.length > 0) {
      console.log(`⚠️  Found ${locationsWithDeletedAt.length} locations with deletedAt that have data:\n`);
      locationsWithDeletedAt.slice(0, 5).forEach(loc => {
        console.log(`   ${loc.locationName}: ${loc.moneyIn.toFixed(2)} (deletedAt: ${loc.deletedAt})`);
      });
    ...
    locationTotals
      ...
      .slice(0, 10)
      .forEach((loc, idx) => {
        console.log(`   ${idx + 1}. ${loc.locationName}: ${loc.moneyIn.toFixed(2)}`);
      });
    console.log();

    console.log('✅ Verification complete\n');
    console.log('💡 The correct total should be WITH deletedAt filtering');
    console.log(`   Expected: $${totalWithDeletedAtFilter.moneyIn.toFixed(2)}\n`);
