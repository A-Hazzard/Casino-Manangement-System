...
async function debug() {
  ...
  try {
    ...
    const yesterdayBase = currentHour < 8
    ...
    for (const licenseeId of allLicenseeIds) {
      const licenseeName = licenseeId
        ? licensees.find(l => l._id.toString() === licenseeId.toString())?.name || 'Unknown'
        : 'Unassigned';
      ...
      console.log(`   Location IDs (strings): ${locationIds.slice(0, 3).join(', ')}${locationIds.length > 3 ? '...' : ''}`);
      ...
      if (machines.length !== machinesWithObjectIds.length) {
        console.log(`   ⚠️  TYPE MISMATCH! String query: ${machines.length}, ObjectId query: ${machinesWithObjectIds.length}`);
      }
    ...
    console.log(`📊 Difference: $${Math.abs(totalMoneyIn - 104.82).toFixed(2)}\n`);
    ...
    console.log(`📊 Raw Total (all locations, no licensee grouping): $${rawMoneyIn.toFixed(2)}`);
    console.log(`📊 Licensee Method Total: $${totalMoneyIn.toFixed(2)}`);
    console.log(`📊 Difference: $${Math.abs(rawMoneyIn - totalMoneyIn).toFixed(2)}`);
