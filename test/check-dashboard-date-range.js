...
async function check() {
  ...
  try {
    ...
    const dashboardAggregation = await db
      .collection('meters')
      .aggregate([
        {
          $match: {
    ...
    console.log(
      `   Money Out: $${(dashboardResult.totalCancelled || 0).toFixed(2)}`
    );
    ...
    const totalInRange =
      (dashboardResult.totalDrop || 0) +
      (outsideResult.totalDrop || 0) +
      (afterResult.totalDrop || 0);
