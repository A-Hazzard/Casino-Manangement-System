...
async function main() {
  ...
  try {
    ...
    const sampleLocations = await db.collection('gaminglocations')
      .find({
        $or: [
          { deletedAt: null },
    ...
    for (const location of sampleLocations) {
      ...
      const machines1 = await db.collection('machines')
        .find({
      ...
      if (machineIds1.length > 0) {
        const meters1 = await db.collection('meters')
          .aggregate([
            {
              $match: {
        ...
        console.log(`   Money In (string query): ${(result1.moneyIn || 0).toFixed(2)}`);
      }

      // Method 2: Check if ObjectId query would find different machines
      try {
        const ObjectId = require('mongodb').ObjectId;
        const machines2 = await db.collection('machines')
          .find({
        ...
        if (machines2.length !== machines1.length) {
          console.log(`   ⚠️  WARNING: ObjectId query found ${machines2.length} machines (different from string query!)`);
        }
      } catch (e) {
      ...
      if (machineIds1.length > 0) {
        const sampleMachine = await db.collection('machines')
          .findOne({ _id: machineIds1[0] }, { projection: { gamingLocation: 1 } });
        
        if (sampleMachine) {
          console.log(`   Machine gamingLocation type: ${typeof sampleMachine.gamingLocation}`);
          console.log(`   Machine gamingLocation value: ${sampleMachine.gamingLocation}`);
          console.log(`   Location ID matches: ${sampleMachine.gamingLocation === locationId}`);
        }
      }
    }
