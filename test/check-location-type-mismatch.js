...
async function check() {
  ...
  try {
    ...
    if (sampleMachine) {
      ...
      console.log(
        `   Is ObjectId: ${sampleMachine.gamingLocation instanceof ObjectId}`
      );
      console.log(
        `   toString(): ${sampleMachine.gamingLocation.toString()}\n`
      );
    }
    ...
    const machinesWithStrings = await db
      .collection('machines')
      .find({
    ...
    if (machinesWithStrings.length !== machinesWithObjectIds.length) {
      ...
      console.log(
        `   ObjectId query: ${machinesWithObjectIds.length} machines`
      );
      console.log(
        `   Missing: ${machinesWithObjectIds.length - machinesWithStrings.length} machines\n`
      );

      // Find which machines are missing
      const stringMachineIds = new Set(
        machinesWithStrings.map(m => m._id.toString())
      );
      ...
      missingMachines.slice(0, 5).forEach(m => {
        console.log(
          `   Machine: ${m._id}, Location: ${m.gamingLocation} (type: ${typeof m.gamingLocation})`
        );
      });
    } else {
    ...
    Object.entries(licenseeCounts).forEach(([key, count]) => {
      console.log(
        `     ${key === 'null' ? 'Unassigned' : key}: ${count} locations`
      );
    });
  } catch (error) {
